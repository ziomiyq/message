...
// Everything works in the browser. Posts are saved with localStorage.
...
function loadThoughts() {
  try {
    const saved = JSON.parse(localStorage.getItem("letoutThoughts"));
    return Array.isArray(saved) ? saved : defaultThoughts;
  } catch {
...
function saveThoughts() {
  localStorage.setItem("letoutThoughts", JSON.stringify(thoughts));
}
...
function renderThoughts() {
  ...
  visible.forEach(thought => {
    ...
    article.innerHTML = `
      ...
      <button class="delete-btn" type="button" data-id="${thought.id}" aria-label="Delete thought">×</button>
    `;
  ...
  thoughtGrid.querySelectorAll(".delete-btn").forEach(button => {
